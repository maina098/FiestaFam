    ...
    if ($("header").hasClass("header-fixed")) {
      ...
      if (nav.length) {
        ...
        if ($("header").hasClass("style-absolute")) {
          ...
          // Instead of inserting a new DOM node (which crashes React),
          // we append it to the body or a safe container, or just add padding to body.
          // Since it's just a placeholder, appending to a non-React managed node is safer,
          // or we can just append it to the nav itself if CSS allows, but appending to body is safest.
          // Actually, we can just use the existing .page class padding or margin.
          // To be safe, we will just not insert it, and add a class to body.
          // Let's just create a div inside the header or append to body.
          // We will use standard React sibling so we do nothing here.
          // The Header component will render <div className="inject-space"></div>
        }

        $(window).on("load scroll", function () {
          if ($(window).scrollTop() > offsetTop + headerHeight) {
            nav.addClass("is-fixed");
            $(".inject-space").show().height(headerHeight1);
          } else {
